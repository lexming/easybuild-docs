...
- working with Git commands on the `easybuild_easyconfigs` repository is much
faster

The archive of old easyconfigs can be easily installed with `pip` by enabling
the _extra_ `archive` option of the `easybuild-easyconfigs` package:
